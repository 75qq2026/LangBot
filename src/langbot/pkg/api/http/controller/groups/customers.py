...
class CustomersRouterGroup(group.RouterGroup):
    async def initialize(self) -> None:
        ...
        async def get_customers() -> str:
            ...
            return self.success(data={'customers': customers, 'total': total, 'limit': limit, 'offset': offset})

        @self.route('/<customer_id>', methods=['GET'], auth_type=group.AuthType.USER_TOKEN)
        async def get_customer(customer_id: str) -> str:
            customer = await self.ap.customer_service.get_customer(customer_id)
            if customer is None:
                return self.http_status(404, -1, 'customer not found')
            return self.success(data={'customer': customer})
        ...
        async def export_customers() -> tuple[quart.Response, int]:
            ...
            file_bytes = await self.ap.customer_service.export_customers_excel(
                keyword=keyword,
                bot_ids=bot_ids if bot_ids else None,
                pipeline_ids=pipeline_ids if pipeline_ids else None,
            )

            response = await quart.make_response(file_bytes)
