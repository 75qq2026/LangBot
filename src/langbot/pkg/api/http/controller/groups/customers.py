...
class CustomersRouterGroup(group.RouterGroup):
    async def initialize(self) -> None:
        ...
        async def list_customers() -> str:
            search = quart.request.args.get('search')
        ...
        async def get_customer(customer_id: str) -> str:
            ...
            if customer is None:
                return self.http_status(404, 404, f'Customer {customer_id} not found')
        ...
        async def export_customers() -> tuple[quart.Response, int]:
            ...
            content = await self.ap.customer_service.export_customers_excel(
                search=search,
                bot_ids=bot_ids if bot_ids else None,
                pipeline_ids=pipeline_ids if pipeline_ids else None,
            )
